...
def perturb_single(data_source, epoch, corpus_test, hybrid=False):
    ...
    with open("./output/%s/entailment.txt" % (args.outf), "a", encoding='utf-8') as ent, \
            open("./output/%s/neutral.txt" % (args.outf), "a") as neu, open("./output/%s/contradiction.txt" % (args.outf), "a") as con :
        for batch in data_source:
            ...
            for i in range(batch_size):
                ...
                try:
                    if int(target[i]) == 0:
                        ent.write(premise_words_ori[i] + "\t" + hypothesis_words_ori[i] + "\t" + str(int(target[i])) + '\n')
                    elif int(target[i]) == 1:
                        neu.write(premise_words_ori[i] + "\t" + hypothesis_words_ori[i] + "\t" + str(int(target[i])) + '\n')
                    elif int(target[i]) == 2:
                        con.write(premise_words_ori[i] + "\t" + hypothesis_words_ori[i] + "\t" + str(int(target[i])) + '\n')
                    ent.flush()
                    neu.flush() 
                    con.flush()
