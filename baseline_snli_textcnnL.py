from models import Baseline_Embeddings, Baseline_LSTM, TextCNN, TextCNNL
...
baseline_model = TextCNNL(100, 300, class_num=3, maxlen=max_len, gpu=False)
model_type = 'textcnn'
baseline_model.load_state_dict(torch.load("./textcnnlast_0.65L.pt"))#  , map_location=torch.device('cpu')
...
if train_mode == True:
    for epoch in range(0, epochs):
        ...
        if curr_acc > best_accuracy:
            print("saving model...")
            with open(save_path + "/" + model_type + '_univae_' + str(curr_acc) + 'L.pt', 'wb') as f:
                torch.save(baseline_model.state_dict(), f)
            best_accuracy = curr_acc

    with open(save_path + "/" + model_type + 'last_' + str(curr_acc)[:4] + 'L.pt', 'wb') as f:
        torch.save(baseline_model.state_dict(), f)
