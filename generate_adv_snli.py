...
from utils import to_gpu, Corpus, SNLIDataset, collate_snli, SNLIDatasetForUniVAE
from models import Baseline_LSTM, Baseline_Embeddings
...
model.load_weights('./latest_model_loss_0.28.weights')
...
def perturb_single(data_source, epoch, corpus_test, hybrid=False):
    ...
    with open("./output/%s/%s_perturbation.txt" % (args.outf, epoch), "a") as f, \
            open("./output/%s/perturbsnli.txt" % (args.outf), "a") as xad :
        for batch in data_source:
            ...
            for i in range(batch_size):
                f.write("\n================Premise==================\n")
                f.write(" ".join(premise_words_ori[i]) + "\n")  # premise 句子
                f.write("===========Original Hypothesis===========\n")
                f.write(" ".join(hypothesis_words_ori[i]) + "\n") # hypothesis 句子
                ...
                x_adv, distance, nearest_x_adv, nearest_dist = search_qqp_snli((premise_words_ori[i], hypothesis_words_ori[i]),\
                                                       target[i], nsamples=50, left=0., high=2.) 
...
if __name__ == '__main__':
    ...
    if args.datatype == 0:
        ...
        print('data set trec')
        corpus_test = TRECDataset(train=False, vocab_size=11000, path='./data/trec6/', maxlen=10,
                                  reset_vocab=voc_uni
                                  )

        testloader = torch.utils.data.DataLoader(corpus_test, batch_size=10,
                                             collate_fn=collate_trec, shuffle=False)
        classifier1 = TRECBaseline_LSTM(100, 300, maxlen=args.maxlen, gpu=False)
        classifier1.load_state_dict(
            torch.load(args.classifier_path + '/baseline/trec_lstm_reset_vocab.pt', map_location=torch.device('cpu'))
        )
    ...
    if args.datatype == 0:
        ...
        perturb_single_trec(test_data, 1, corpus_test, hybrid=True)
